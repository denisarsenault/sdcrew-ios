...
//  Forked by Bruno Miguens on 30/05/2017.
...
class BMSegmentedControl: UIControl {
    ...
    fileprivate func getIconFrameByOrientation(_ orientation: ComponentOrientation, index: Int, text: String) -> CGRect {
        ...
        let iconX = getIconRightX(width, textWidth: evaluateStringWidth(text), textStart: getTextFrameByOrintation(componentOrientation, text: text, index: index).origin.x)
        let x = width * CGFloat(index)
        
        switch orientation {
        case .leftRight:
            let evaluateIconX = iconX
            let iconRect = CGRect(x: evaluateIconX, y: 0, width: 32, height: height)
            return iconRect
        case .topDown:
    ...
    fileprivate func getTextFrameByOrintation(_ orientation: ComponentOrientation, text: String, index: Int) -> CGRect {
        ...
        let textX = getTextLeftX(width, textWidth: evaluateStringWidth(text))
    ...
    fileprivate func getIconRightX(_ itemWidth: CGFloat, textWidth: CGFloat, textStart: CGFloat) -> CGFloat {
        let iconWidth: CGFloat = 16.0
        let avg = iconWidth + textWidth
        let space: CGFloat = (itemWidth + avg) / 2
        
        return textWidth + textStart
    }
    ...
    fileprivate func getTextLeftX(_ itemWidth: CGFloat, textWidth: CGFloat) -> CGFloat {
        let iconWidth: CGFloat = withIcon ? 16.0 : 0.0
        let avg = iconWidth + textWidth
        let space: CGFloat = (itemWidth - avg) / 2
    ...
    internal override func layoutSubviews() {
        ...
        thumbView.addBottomBorderWithColor(color: thumbColor, width: 2)
    ...
    internal override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        for (index, item) in labels.enumerated() {
            ...
            let iconX = getIconRightX(labelWidth, textWidth: evaluateStringWidth(text), textStart: item.frame.origin.x)
            
            let itemStarts = self.frame.width / CGFloat(items.count) * CGFloat(index)
            
            let frame = CGRect(x: itemStarts , y: 0, width: item.frame.width, height: self.bounds.height)
    ...
    fileprivate func displayNewSelectedIndex() {
        ...
        let labelWidth = self.bounds.width / CGFloat(items.count)
        let iconX = getTextLeftX(labelWidth, textWidth: evaluateStringWidth(text))
