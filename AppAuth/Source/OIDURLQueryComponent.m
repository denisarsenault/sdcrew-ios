...
- (nullable instancetype)initWithURL:(NSURL *)URL {
  ...
  if (self) {
    if (@available(iOS 8.0, macOS 10.10, *)) {
      NSURL *uurl = [NSURL URLWithString: [[URL absoluteString] stringByReplacingOccurrencesOfString:@"#" withString:@"?"]];
      // If NSURLQueryItem is available, use it for deconstructing the new URL. (iOS 8+)
      if (!gOIDURLQueryComponentForceIOS7Handling) {
        NSURLComponents *components =
            [NSURLComponents componentsWithURL:uurl resolvingAgainstBaseURL:NO];
