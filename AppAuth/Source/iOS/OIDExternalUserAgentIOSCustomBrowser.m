...
- (BOOL)presentExternalUserAgentRequest:(nonnull id<OIDExternalUserAgentRequest>)request
  ...
  BOOL openedInBrowser = [[UIApplication sharedApplication] openURL:requestURL options:@{} completionHandler:(void (^)(BOOL success)) {
    if (success) {
    }
...
- (void)dismissExternalUserAgentnimated:(BOOL)animated
